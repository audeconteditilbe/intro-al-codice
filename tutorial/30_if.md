[<- lezione precedente (variabili)](./20_variabili.md)

[-> lezione successiva (cicli)](./40_cicli.md)
...
### Esempio (confronto fra variabili)
...
[lo scorso episodio](./20_variabili.md#booleani).
...
In questo caso, entrambe le condizioni sono rispettate, quindi il valore di `canRider` è `true`.
...
```
se [CONDIZIONE] alloa [OPERAZIOINI] altrimenti [OPERAZIONI]
```
...
if ( CONDIZIONE ) { // se CONDIZIONE si verifica...
  ...
  // ...altrimenti fai quello che trovi qui
}
```
...
1) La condizione specificata nell'`if` è l'espressione booleana `num1 === num2`.
2) Questa risolve a `false`, quindi le istruzioni specificate fra le graffe dell'`if` non sono eseguite.
3) Le operazioni specificate fra le graffe dell'`else` sono quindi eseguite.
...
```javascript
if ( CONDIZIONE ) {
  // se CONDIZIONE si verifica...
}
else if ( CONDIZIONE_2 ) {
  // altrimenti, se CONDIZIONE_2 si verifica...
}
else if ( CONDIZIONE_3 ) {
  // altrimenti, se CONDIZIONE_3 si verifica...
}
...
else {
...
Se nessuna condizione si verifica, viene eseguito il codice dell'`else`.
...
Questo esempio stamperà "Non puoi guidare ubriaco!". <s>(lol prova a fermarmi)</s>
...
Osservazioni:
  - l'assenza dell'`else` finale. Non è obbligatorio infatti! Tanto sotto i 3 anni non capiscono che li stai salutando 💁🏻‍♀️
...
Prendiamo l'esempio precedente, e trasformiamo gli `else if` in semplici `if` messi uno dopo l'altro;
